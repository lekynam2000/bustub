...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if(args.size()!=1){
    throw Exception(fmt::format("func call {} not supported with {} arguments in planner yet", func_name, args.size()));
  }
  if(func_name=="upper"){
    return std::make_shared<bustub::StringExpression>(args[0], bustub::StringExpressionType::Upper);
  }
  
  return std::make_shared<bustub::StringExpression>(args[0], bustub::StringExpressionType::Lower);
